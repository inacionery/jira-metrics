  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.$watch('stats', function(newValue, oldValue) {
        ...
        var svg = d3.select('#' + element[0].id).append("svg").attr("width", width).attr("height", height).append("g")
        ...
        var line = d3.svg.line().x(function(d) {
          return x(d.weekNumber);
        }).y(function(d) {
          return height - y(d.throughput);
        });
        ...
        _.each(scope.stats, function(stat) {
          if (thisWeek === stat.week) {
            ...
            for (var i = 0; i < stat.throughput.length; i++) {
              ...
              data.push({weekNumber: i, throughput: throughputValue});
            };
            ...
            svg.append("path").datum(data).attr("class", "line").attr("d", line).append("svg:title").text(stat.throughput);
          }
        });
